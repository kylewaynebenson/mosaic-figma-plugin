    ...
    if ('children' in node && node.children.length > 0) {
        ...
        for (const child of node.children) {
            if (hasFills(child)) {
                ...
                if (solidFills.length > 0) {
                    totalR += solidFills[0].color.r;
                    totalG += solidFills[0].color.g;
                    totalB += solidFills[0].color.b;
                    count++;
                }
            }
        }
...
function getSampleColorAtPoint(node, x, y, width, height) {
...
function getSampleColorData(node, x, y, width, height, resolution) {
    return __awaiter(this, void 0, void 0, function* () {
        try {
...
function getComponentColorData(component, resolution) {
    try {
        const colorData = [];
...
function calculateColorDifference(data1, data2, bailOutValue = Infinity) {
    let diff = 0;
...
function findBestMatchComponent(sourceColor, componentLibrary) {
    let bestMatch = null;
    ...
    shuffledComponents.forEach(component => {
        ...
        const difference = Math.sqrt(Math.pow(sourceColor.r - componentColor.r, 2) +
...
function findBestMatchingTile(imageSection, componentLibrary, resolution) {
    return __awaiter(this, void 0, void 0, function* () {
        let bestMatchIndex = 0;
        let minDiff = Number.MAX_VALUE;
        // Pre-compute all component color data
        const componentColorData = componentLibrary.map(component => getComponentColorData(component, resolution));
...
function createMosaic(selectedImage, componentLibrary, config) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        mosaicGroup.resize(selectedImage.width * config.enlargement, selectedImage.height * config.enlargement);
        ...
        for (let x = 0; x < xTileCount; x++) {
            for (let y = 0; y < yTileCount; y++) {
                ...
                let imageSection;
                if (selectedImage.type === 'RECTANGLE' || selectedImage.type === 'FRAME') {
                    imageSection = yield getSampleColorData(selectedImage, sampleX, sampleY, sampleWidth, sampleHeight, config.matchResolution);
                }
                else {
                ...
                let bestMatchComponent = componentLibrary[0];
                let minDiff = Number.MAX_VALUE;
...
function getLibraries() {
    const libraries = figma.root.children
...
function getAllComponentVariants(scope, useVariants = true) {
    const components = [];
    ...
    if ('findAll' in scope && 'findChildren' in scope) {
        ...
        const supportedNode = scope;
        if (useVariants) {
            try {
                // Find all component sets (variant collections)
                const componentSets = supportedNode.findAll(node => node.type === 'COMPONENT_SET');
        ...
        try {
            ...
            const standaloneComponents = supportedNode.findAll(node => {
                var _a;
                return node.type === 'COMPONENT' &&
                    (!useVariants || ((_a = node.parent) === null || _a === void 0 ? void 0 : _a.type) !== 'COMPONENT_SET');
            });
...
figma.ui.onmessage = (msg) => __awaiter(this, void 0, void 0, function* () {
    ...
    if (msg.type === 'create-mosaic') {
        ...
        if (config.componentSource === 'current-page') {
            ...
            componentLibrary = getAllComponentVariants(figma.currentPage, config.useComponentVariants).filter(component => Math.abs(component.width - config.tileSize) < 0.1 &&
            ...
            const library = figma.root.children.find((page) => page.type === 'PAGE' &&
                page.id === config.libraryName);
            if (library) {
                componentLibrary = getAllComponentVariants(library, config.useComponentVariants).filter(component => Math.abs(component.width - config.tileSize) < 0.1 &&
                    Math.abs(component.height - config.tileSize) < 0.1);
            }
        }
        ...
        yield createMosaic(selection[0], componentLibrary, config);
    }
});
